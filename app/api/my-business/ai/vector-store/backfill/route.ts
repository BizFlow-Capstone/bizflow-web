...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(
      `${AI_BACKEND_API_URL}/api/my-business/ai/vector-store/backfill`,
    );
    ...
    console.error("Error backfilling AI vector store:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to backfill AI vector store",
      },
      { status: 500 },
    );
  }
