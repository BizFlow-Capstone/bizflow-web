...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(
      `${AI_BACKEND_API_URL}/api/my-business/ai/anomalies`,
    );
    backendUrl.searchParams.set("locationId", locationId);

    if (acknowledged !== null && acknowledged !== "") {
      backendUrl.searchParams.set("acknowledged", acknowledged);
    }
    ...
    console.error("Error fetching AI anomaly alerts:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch AI anomaly alerts",
      },
      { status: 500 },
    );
  }
