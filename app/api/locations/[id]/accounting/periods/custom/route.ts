...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(
      `${BACKEND_API_URL}/api/locations/${id}/accounting/periods/custom`,
      {
        method: "POST",
        headers: {
    ...
    console.error("Error creating custom accounting period:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to create custom accounting period",
      },
      { status: 500 },
    );
  }
